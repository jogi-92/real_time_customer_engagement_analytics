    ...
    for job in jobs:
        ...
        if DATAFLOW_JOB_NAME in job_name:
            ...
            if state == "JOB_STATE_RUNNING":
                ...
                print("Dataflow Streaming Job is RUNNING")
...
with DAG(
    ...
    start_expansion_service = BashOperator(

        task_id='start_expansion_service',

        bash_command="""
        if netstat -tulnp | grep -q 12345; then
            echo "Expansion Service already running"
        else
            nohup java -jar \
            ~/.apache_beam/cache/jars/beam-sdks-java-io-expansion-service-2.73.0.jar \
            12345 \
            > /tmp/expansion.log 2>&1 &

            sleep 20
        fi

        netstat -tulnp | grep 12345 || true
        """
    )
    ...
    start_generator = BashOperator(
        ...
        nohup python3 event_generator1.py \
        > /tmp/generator.log 2>&1 < /dev/null &
    ...
    start_enricher = BashOperator(
        ...
        nohup python3 event_enricher1.py \
        > /tmp/enricher.log 2>&1 < /dev/null &
    ...
    start_beam_pipeline = BashOperator(

        task_id='start_beam_pipeline',

        bash_command="""
        cd ~

        pkill -f beam_pipeline1.py || true

        nohup ~/airflow-env/bin/python \
        ~/beam_pipeline1.py \
        > /tmp/beam.log 2>&1 < /dev/null &

        sleep 60

        echo "Beam Pipeline Submitted"
        """
    )
